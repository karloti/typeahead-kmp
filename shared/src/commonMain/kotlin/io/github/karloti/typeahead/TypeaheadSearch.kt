...
interface TypeaheadSearch<T, K> {
     ...
     *
     * The output is a JSON array where the first element is a metadata header
     * and all subsequent elements are payload records with pre-computed vectors.
     ...
     */
    fun exportToSink(sink: Sink, itemSerializer: KSerializer<T>, json: Json)

    /**
     * Restores the engine state by streaming records from a [Source], bypassing vectorization.
     *
     * Deserializes a JSON array previously produced by [exportToSink]. The first element
     * is expected to be metadata; subsequent elements are payload records whose pre-computed
     * vectors are loaded directly into the index.
     *
     * ```kotlin
     ...
     * ```
     *
     * @param A The type of the serialized items (must match [T] at runtime).
     * @param source The [Source] to read the JSON array from.
     * @param itemSerializer The [KSerializer] for item deserialization.
     * @param clearExisting If `true`, the current index is replaced; if `false`, imported records are merged.
     * @param json The [Json] instance used for deserialization.
     * @throws IllegalStateException if the imported metadata has an incompatible N-gram size.
     */
    fun <A> importFromSource(
        source: Source,
        itemSerializer: KSerializer<A>,
...
/**
 * Convenience extension that infers the [KSerializer] from the reified type parameter [A].
 *
 * ```kotlin
 * val engine: TypeaheadSearch<String, String> = ...
 * fileSystem.source(path).use { source ->
 ...
 * @throws IllegalStateException if the imported metadata has an incompatible N-gram size.
 */
inline fun <reified A> TypeaheadSearch<*, *>.importFromSource(
    source: Source,
...
) = importFromSource(source, serializer<A>(), clearExisting, json)
